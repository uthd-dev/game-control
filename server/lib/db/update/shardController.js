  ...
  User.findOneAndUpdate(
    ...
    function (err, result) {
      if (err) {
        ...
        console.log(
          `Success! Set the user: ${twitchUser.name}'s shards to ${numberOfShards}`
        );
        console.log(result);
        return done(result);
      }
...
const addShards = (twitchUser, numberOfShards, done) => {
  ...
  User.findOneAndUpdate(
    { twitchId: twitchUser.twitchId },
    { shards: newNumberOfShards },
    function (err, result) {
      if (err) {
        console.log(
          `Error! Could not add ${numberOfShards} shards for user: ${twitchUser.name}`
        );
        console.log(err);
        return done(err);
      } else {
        console.log(
          `Success! Added ${numberOfShards} to ${twitchUser.name}'s balance`
        );
        console.log(result);
        return done(result);
      }
  ...
  if (twitchUser.shards >= numberOfShards) {
    ...
    User.findOneAndUpdate(
      ...
      function (err, result) {
        if (err) {
          console.log(
            `Error! Could not spend ${numberOfShards} shards for user: ${twitchUser.name}`
          );
          console.log(err);
          return done(
            false,
            `An error occurred while processing your request, please try again later and let us know this happened!`
          );
        } else {
          console.log(
            `Success! Spent ${numberOfShards} from ${twitchUser.name}'s balance`
          );
          console.log(result);
          return done(
            true,
            `Woohoo! Thank you? Depends on what you sent... Uh Oh`
          );
        }
      }
    );
