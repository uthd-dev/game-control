...
async function addStreamer (user, streamerInfo, done) {
    ...
    else if(!streamerInfo.tel) await User.findOneAndUpdate({'twitchId': user.twitchId }, { "$set": {"fname": streamerInfo.fname, "ign": streamerInfo.ign} },  (err) => {
        if(err) finish(true, user, done);
        else finish(false, user, done);
    });
}
async function finish (err, user, done)  {
    if(err) {
        console.log(`Error! Could not add Streamer ${user.displayName} to Users DB.`); //I love backtick concantenation, wait...
        console.log(err);

        //Custom Callback function
        return done(false);
    }else {
        console.log(`Success! Added the streamer, ${user.displayName} to Users DB.`);
